...
library(CAST)
library(doParallel)
data<-readRDS("DATA/training_data/training_polygons12N_02122020_BS_UPDATE3.rds")%>%
  drop_na()%>%
  mutate(class = case_when(
    class %in% c(7,2) ~ "Sage", #Sage and Black Sage 
    class %in% c(4,5) ~ "Other_veg", #Other_veg and other_shrub,
...
saveRDS(caret_fit_base, "model_runs/ID_CV_model_BS_UPDATE/ID_CV_model_BS_UPDATE3.rds")
